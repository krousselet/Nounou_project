...
const page = {
  ...
  init: function (app) {
    ...
    if (document.querySelector(".btn_d_addChild")) {
      document
        .querySelector(".btn_d_addChild")
        .addEventListener("click", () => {
          let form = panda.util.newelem("form", {
            className:
              "form_d_addChild  d-flex flex-column justify-content-center align-items-center mt-2",
          });
          form.appendChild(
            panda.util.newelem("h1", {
              textContent: "Ajouter un enfant",
              className: "fs-5 text-center mt-2 mb-2",
            })
          );
          form.appendChild(
          ...
          document
            ...
            .addEventListener("click", (e) => {
              ...
              const prenom = forme.querySelector(
                "input[name=Child_prenom]"
              ).value;
    ...
    if (document.querySelector(".btn_d_listChild")) {
      document
        .querySelector(".btn_d_listChild")
        .addEventListener("click", () => {
          let childsdiv = panda.util.newelem("div", { className: "" });
          childsdiv.appendChild(
            panda.util.newelem("h1", {
              textContent: "Liste des enfants",
              className: "fs-5 text-center mt-2 mb-2",
            })
          );
          panda.ajax(
            ...
            (data) => {
              ...
              childs.forEach((element) => {
                let child = panda.util.newelem("div", {
                  className: "d-flex gap-2",
                }); //Nom Prenom Age
                child.appendChild(
                  panda.util.newelem("p", {
                    textContent: element.nom,
                    className: "fs-5 text-center mt-2 mb-2",
                  })
                );
                child.appendChild(
                  panda.util.newelem("p", {
                    textContent: element.prenom,
                    className: "fs-5 text-center mt-2 mb-2",
                  })
                );
                child.appendChild(
                  panda.util.newelem("p", {
                    textContent: element.age,
                    className: "fs-5 text-center mt-2 mb-2",
                  })
                );
                child.appendChild(
    ...
    if (document.querySelector(".btn_d_DispoNounou")) {
      document
        .querySelector(".btn_d_DispoNounou")
        .addEventListener("click", () => {
          document
            .querySelector(".blur")
            .querySelectorAll("& > *")
    ...
    let calendar = new FullCalendar.Calendar(calendarEl, {
      ...
      select: function (info) {
        ...
        if (info.startStr == info.endStr) {
          ...
          document.querySelector("input[name=eventDateStart]").value =
            h_1 + ":" + m_1;
          document.querySelector("input[name=eventDateEnd]").value =
            h_2 + ":" + m_2;
        }
      },
    });
      ...
      (data) => {
        ...
        if (planing.length > 0) {
          planing.forEach((element) => {
            let list = {
              title: "Disponibilité - " + element.nom + " " + element.prenom,
            };
            list.startRecur = element["jour_start"];
  ...
  ValidReserv: function (info) {
    ...
    let form = panda.util.newelem("form", {
      className:
        "form-group reserve-form card form-log-reg pt-3 ps-3 pe-3 pb-3 ",
    });
    panda.ajax("./ajax/dashboard.php", { action: "getListChild" }, (data) => {
      ...
      form.appendChild(
        panda.util.newelem("h1", {
          textContent: "Validation de la réservation",
          className: "fs-4 text-center mt-2 mb-2 color-font-dark",
        })
      );

      form.appendChild(
        panda.util.newelem("p", {
          textContent: "Nounou : " + event.extendedProps.name,
          className: "fs-4 text-center mt-2 mb-2 color-font-dark text-shadow",
        })
      );

      form.appendChild(
        panda.util.newelem("p", {
          textContent: "Veuillez choisir les enfants pour la réservation: ",
          className: "fs-5 text-center mt-2 mb-3 color-font-dark",
        })
      );
      ...
      childs.forEach((element) => {
        form.appendChild(
          panda.util.newelem("input", {
            type: "checkbox",
            id: "child_" + element.Id_Enfants,
            name: "child_" + element.Id_Enfants,
            style: "display:none",
          })
        );
        form.appendChild(
          panda.util.newelem("label", {
            htmlFor: "child_" + element.Id_Enfants,
            textContent: element.nom + " " + element.prenom,
          })
        );
      });
      ...
      form.appendChild(
        panda.util.newelem("label", {
          htmlFor: "allweek",
          textContent: "Toutes les semaines",
        })
      );
      let validbutton = panda.util.newelem("input", {
        type: "submit",
        value: "Valider la réservation",
        className: "btn btn-custom-color mt-3 mb-2",
      });
      ...
      validbutton.addEventListener("click", (e) => {
        ...
        panda.ajax(
          "./ajax/dashboard.php",
          {
            ...
            Id: event.id,
            allweek: document.getElementById("allweek").checked,
            children: children,
          },
          (data) => {
