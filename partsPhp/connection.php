<div class="container d-flex justify-content-center align-items-center">

  <div class=" form-log-reg card searchBars d-flex flex-column col-4 m-5">
  <h1 class="text-center">Connection</h1>

  <form method="POST" class="d-flex flex-column ">

 <label class="connect-label">Adresse Mail</label>
 <input class="connect-input" placeholder="Adresse Mail" type="text" id="mail-connect" name="mail-connect" >

 <label class="connect-label">Mot de passe</label>
 <input class="connect-input" placeholder="Mot de passe" type="text" id="mdp-connect" name="mdp-connect" >

<button id="btnSearch"  class="btn btn-custom-color m-3" value="search" id="Login">Se connecter</button>

</form>


  </div>
</div>